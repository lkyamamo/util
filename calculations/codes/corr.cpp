...
struct NeighborList
{
	...
	nbrlist_t nbrlist_hcenter_long; 
	...
	NeighborList(MDFrame & mdframe, int _max=MAX_NEIGHBOR_MOLS) : max_neighbors(_max), num_atoms(mdframe.natoms)
	{
		...
		nbrlist_hcenter_long.resize(mdframe.natoms);	
		...
		for (int i=0; i<mdframe.natoms; i++)
		{
			...
			for (int j1=0; j1<idlist.size(); j1++)
			{
				...
				//// ignore if an atomic pair distance is beyond h-bond outer cutoff
				...
				if(dr < HBOND_OUTER_CUTOFF && is_i_oxygen && not is_j_oxygen) nbrlist_ncenter_long[i].push_back(data);
				if(dr < HBOND_OUTER_CUTOFF && not is_i_oxygen && is_j_oxygen) nbrlist_hcenter_long[i].push_back(data);
			}
		}
		...
		for (std::vector<tuple_t> & n : nbrlist_ncenter_long)
			std::sort(begin(n), end(n), [](auto const & t1, auto const & t2) { return std::get<0>(t1) < std::get<0>(t2);} );
		// sort hcenter_long list by distance
		for (std::vector<tuple_t> & n : nbrlist_hcenter_long)
			std::sort(begin(n), end(n), [](auto const & t1, auto const & t2) { return std::get<0>(t1) < std::get<0>(t2);} );
	}
...
struct rotational_correlations
{
	...
	auto get_normalized_vectors_h2o(const std::vector<tuple_t> & n)
	{
		// H1 
		int jgid = std::get<1>(n[0]); 
		const auto nj_r = std::get<3>(n[0]);
		...
		// H3 
		int lgid = kgid; 
		const auto nl_r = nk_r;
		const double nl_norm = nk_norm;

		std::vector<double> e1 = {nj_r.dx, nj_r.dy, nj_r.dz}; 
		std::vector<double> e2 = {nj_r.dx-nk_r.dx, nj_r.dy-nk_r.dy, nj_r.dz-nk_r.dz};
		std::vector<double> e3 = {nj_r.dx-nk_r.dx, nj_r.dy-nk_r.dy, nj_r.dz-nk_r.dz};
	...
	void register_ref_vectors(nbrlist_t const & nbrlist_ncenter)
	{
		...
		for (int iatom = 0; iatom< nbrlist_ncenter.size(); iatom++)
		{
			...
			if(n.size() >= 3) 
			{
				...
				auto vectors = get_normalized_vectors_h2o(n);
				e1_ref[iatom] = std::get<0>(vectors);
				e2_ref[iatom] = std::get<1>(vectors);
				e3_ref[iatom] = std::get<1>(vectors);
			}
		}
	...
	void compute_correlations(int iframe, nbrlist_t const & nbrlist_ncenter)
	{
		for (int ts = 0; ts < e1_refs.size(); ts++)
		{
			...
			for (int ia = 0; ia< nbrlist_ncenter.size(); ia++)
			{
				...
				else if(n.size() == 2) // For H2O molecule
				{
					bool e1_flag = e1_refs[ts].find(ia) != e1_refs[ts].end();
					bool e2_flag = e2_refs[ts].find(ia) != e2_refs[ts].end();
					bool e3_flag = e3_refs[ts].find(ia) != e3_refs[ts].end();
...
auto angle_O1HO2 = make_histogram(axis::regular<double>(180, 0.0, 180.0));
auto angle_H3OH4 = make_histogram(axis::regular<double>(180, 0.0, 180.0));
				...
				*/
				...
				for (int iatom = 0; iatom< nbr.nbrlist_ncenter_long.size(); iatom++)
				{
					...
					if(n.size() >= 4) 
					{
						...
						if(tseries_id==0) angle_H3OH4(degree); // once in the tseries loop
					}
				}

				for (int iatom = 0; iatom< nbr.nbrlist_hcenter_long.size(); iatom++)
				{
					const auto n = nbr.nbrlist_hcenter_long[iatom]; 
					// O1-H-O2 angle
					if(n.size() >= 2) 
					{
						int j = 0;  // O1
						...
						if(tseries_id==0) angle_O1HO2(degree); // once in the tseries loop
					}
				}
...
//		hb_prof.save();
	...
	void save_result()
	{
		...
		auto all_histograms = {angle_H3OH4, angle_O1HO2};
		...
		fout_histogram << "angle    H3-O-H4    O1-H-O2 " << std::endl;
