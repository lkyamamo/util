...
std::map<std::string, double> CHARGE{{"O",-2}, {"H",1}, {"N",-3}, {"Na",1}, {"Li",1}, {"K", 1}, {"Si", 4}};
...
struct MSD
{
	...
	num_total_atoms = frame.natoms;
	num_atoms.resize(index.size(),0);
        for(int i=0; i<frame.natoms; i++)
        {
            auto & name = frame.name[i];
	    num_atoms[index[name]]++; 
        }
        ...
        for (auto it = index.begin(); it != index.end(); ++it)
            std::cout << "element,num,id,charge: " << 
		    it->first << " " << num_atoms[it->second] << " " << 
		    it->second << " " << CHARGE[it->first] << std::endl ;
    ...
    void get_msd(std::vector<MDFrame> &frames)
    {
		...
		std::cout << " time " << t << " ";
            ...
            {
		    std::cout << samples[type][t]/num_measurements << " "; 
                if(samples[type][t]>0)
                {
	    ...
	    std::cout << std::endl;
            ...
            Jt[t] = Jt[t]*cos(0.5*M_PI*(double)t/corr_length);
            Jt[t] /= J0;
            outfile << Jt[t];
        ...
        for(int i=0; i<num_grids; i++) // w
        {
            ...
            {
                outfile << Gw[type][i] << " ";
		double Nalpha = (double)num_atoms[type]/num_total_atoms;
                Gw_total += 6.0*Nalpha/M_PI*Gw[type][i];
            }
            outfile << Gw_total << " "; 
...
int main(int argc, char *argv[])
{
		...
		while(!ifile.eof())
		{
			...
			std::cout << "mdframes.size() " << mdframes.size() << std::endl;
		}

        print_mdframe(mdframes[0]);
