...
def process_dipole_data(file1, averaging_function, data_key_prefix, increment):
    """
...
def process_dipole_data_binned(file1, bin_size=BIN_SIZE, increment=INCREMENT):
    """
...
if AVERAGING_METHOD == 'binned':
    ...
    timestep_data, mavg, msq, counter, mxx, myy, mzz = process_dipole_data_binned(file1, increment=increment)
else:
    ...
    timestep_data, mavg, msq, counter, minitial, mxx, myy, mzz = process_dipole_data(
        file1, averaging_function, data_key_prefix, increment=increment
    )
