...
!-----------------------------------------------------------------------------------------
     ...
     do i=1, size(oneframe%elems)
        if(scan(sbuf,trim(adjustl(oneframe%elems(i)%str)))==0) then
           sbuf=sbuf//trim(adjustl(oneframe%elems(i)%str))//'@'
           num_elems=num_elems+1
        endif
     enddo
     print'(a,i3)',sbuf, num_elems

     allocate(c%elems(num_elems))
     do i=1, size(c%elems)
        c%elems(i)%str = sbuf(1:scan(sbuf,'@')-1)
        sbuf=sbuf(scan(sbuf,'@')+1:) ! trim to first @ mark.
     enddo
