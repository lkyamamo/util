#!/usr/bin/env python3
"""
MPI-parallel version for collecting bond statistics from molecular dynamics trajectories.
...
Usage:
    mpirun -np 64 python process_bonds_stats_mpi.py <input_file> <start> <end> <increment> 
...
def process_frame(atoms, num_atoms, timestep, type_A, type_B, cutoff, box_dims):
    ...
    return timestep, len(bonds), bond_stats
...
def read_bond_stats_file(filename):
    """Read a bond statistics output file and return a dictionary of {timestep: stats}."""
    results = {}
                    ...
                    if len(parts) >= 7:
                        try:
                            ...
                            stats = {
                                ...
                                'bonds_created': int(parts[2]),
                                'bonds_missing': int(parts[3]),
                                'atoms_within_cutoff': int(parts[4]),
                                'type_A_count': int(parts[5]),
                                'type_B_count': int(parts[6])
                            }
                            results[timestep] = stats
...
def gather_running_statistics(running_stats, comm, rank, size):
    """
    Gather running statistics from all MPI processes and calculate global averages.
    
    Returns:
        dict: Global statistics including averages, std devs, min/max, etc.
    """
    # Convert lists to numpy arrays for easier calculation
    local_stats = {}
    for key, values in running_stats.items():
        if values:  # Only process if we have data
            local_stats[key] = np.array(values)
        else:
            local_stats[key] = np.array([])
    
    # Gather all statistics from all processes
    all_stats = {}
    for key in ['num_bonds', 'bonds_created', 'bonds_missing', 'atoms_within_cutoff', 'type_A_count', 'type_B_count']:
        # Gather the data from all processes
        gathered_data = comm.gather(local_stats[key], root=0)
        
        if rank == 0:
            # Combine all data from all processes
            all_data = np.concatenate([data for data in gathered_data if len(data) > 0])
            all_stats[key] = all_data
    
    if rank == 0 and all_stats:
        # Calculate global statistics
        total_frames = len(all_stats['num_bonds'])
        ...
        for key, data in all_stats.items():
            if len(data) > 0:
                avg_stats[key] = np.mean(data)
                std_stats[key] = np.std(data)
                min_stats[key] = np.min(data)
                max_stats[key] = np.max(data)
            else:
                avg_stats[key] = 0.0
                std_stats[key] = 0.0
                min_stats[key] = 0
        ...
        # Calculate bond formation success rate
        total_possible_bonds = np.sum(all_stats['bonds_created']) + np.sum(all_stats['bonds_missing'])
        success_rate = (np.sum(all_stats['bonds_created']) / total_possible_bonds * 100) if total_possible_bonds > 0 else 0.0
        
        # Calculate bond density
        avg_bond_density = (avg_stats['num_bonds'] / avg_stats['type_A_count']) if avg_stats['type_A_count'] > 0 else 0.0
        ...
        return {
            ...
            'total_possible_bonds': total_possible_bonds,
            'total_created': np.sum(all_stats['bonds_created']),
            'total_missing': np.sum(all_stats['bonds_missing']),
            'bond_density': avg_bond_density
        }
    else:
        return None
...
def process_concatenated_file_mpi(filename, start, end, increment, type_A, type_B, 
    ...
    - Running statistics: tracks averages during processing
    """
    ...
    if rank == 0:
        print("=" * 70)
        print("MPI Parallel Bond Statistics Collection - 3-Atom Molecular Bonds")
        print("=" * 70)
    ...
    # Initialize running statistics tracking
    running_stats = {
        'num_bonds': [],
        'bonds_created': [],
        'bonds_missing': [],
                    ...
                    if timestep_index % size == rank:
                        try:
                            ...
                            if result is not None:
                                ...
                                running_stats['num_bonds'].append(num_bonds)
                                running_stats['bonds_created'].append(bond_stats['bonds_created'])
                                running_stats['bonds_missing'].append(bond_stats['bonds_missing'])
                                running_stats['atoms_within_cutoff'].append(bond_stats['atoms_within_cutoff'])
                                running_stats['type_A_count'].append(bond_stats['type_A_count'])
                                running_stats['type_B_count'].append(bond_stats['type_B_count'])
    ...
    global_stats = gather_running_statistics(running_stats, comm, rank, size)
    
    if rank == 0 and global_stats:
        print(f"Total frames processed: {global_stats['total_frames']}")
        print(f"")
        print(f"AVERAGE VALUES (mean ± std):")
        print(f"  Number of bonds:        {global_stats['averages']['num_bonds']:8.2f} ± {global_stats['std_devs']['num_bonds']:6.2f}")
        print(f"  Bonds created:          {global_stats['averages']['bonds_created']:8.2f} ± {global_stats['std_devs']['bonds_created']:6.2f}")
        print(f"  Bonds missing:          {global_stats['averages']['bonds_missing']:8.2f} ± {global_stats['std_devs']['bonds_missing']:6.2f}")
        print(f"  Atoms within cutoff:    {global_stats['averages']['atoms_within_cutoff']:8.2f} ± {global_stats['std_devs']['atoms_within_cutoff']:6.2f}")
        print(f"  Type A atoms:           {global_stats['averages']['type_A_count']:8.2f} ± {global_stats['std_devs']['type_A_count']:6.2f}")
        print(f"  Type B atoms:           {global_stats['averages']['type_B_count']:8.2f} ± {global_stats['std_devs']['type_B_count']:6.2f}")
        print(f"")
        print(f"RANGE VALUES (min - max):")
        print(f"  Number of bonds:        {global_stats['mins']['num_bonds']:8.0f} - {global_stats['maxs']['num_bonds']:8.0f}")
        print(f"  Bonds created:          {global_stats['mins']['bonds_created']:8.0f} - {global_stats['maxs']['bonds_created']:8.0f}")
        print(f"  Bonds missing:          {global_stats['mins']['bonds_missing']:8.0f} - {global_stats['maxs']['bonds_missing']:8.0f}")
        print(f"")
        print(f"BOND FORMATION SUCCESS RATE:")
        print(f"  Successfully created:  {global_stats['total_created']:8.0f} / {global_stats['total_possible_bonds']:8.0f} ({global_stats['success_rate']:6.2f}%)")
        print(f"  Failed to create:      {global_stats['total_missing']:8.0f} / {global_stats['total_possible_bonds']:8.0f} ({100-global_stats['success_rate']:6.2f}%)")
        print(f"")
        print(f"BOND DENSITY:")
        print(f"  Average bonds per type A atom: {global_stats['bond_density']:8.2f}")
        print("="*70)
    elif rank == 0:
...
if __name__ == "__main__":
    if len(sys.argv) < 12:
        if rank == 0:
            print("Usage:")
            print("  mpirun -np <num_procs> python process_bonds_stats_mpi.py <input_file> <start> <end> <increment> "
                  "<type_A> <type_B> <cutoff> <la> <lb> <lc> <output_file>")
            print("\nExample:")
            print("  mpirun -np 64 python process_bonds_stats_mpi.py trajectory.dat 0 1000 10 1 2 1.2 "
                  "35.0 35.0 35.0 bond_stats_output.txt")
            print("\nNote:")
